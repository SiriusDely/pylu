...
def order_points(pts):
    ...
    # the top-left point will have the smallest sum, where-as
    # the bottom-right point will have the largest sum
    s = pts.sum(axis=1)
    rect[0] = pts[np.argmin(s)]
    rect[2] = pts[np.argmax(s)]

    # now compute the difference between to points, the
    # top-right point will have the smallest difference,
    # where-as the bottom-left will have the largest difference
...
def four_point_transform(image, pts):
    # obtain a consistent order of the points and unpack them indiviually
