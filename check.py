...
def extract_digits_and_symbols(image, charContours, minW=5, minH=15):
    ...
    # lists, respectively
    charIter = charContours.__iter__()
    rois = []
    locs = []
    ...
    while True:
        try:
            ...
            # otherwise, we are examining one of the special symbols
                ...
                parts = [contour, next(charIter), next(charIter)]
                (sXA, sYA, sXB, sYB) = (np.inf, np.inf, -np.inf, -np.inf)

                # loop over the parts
                for p in parts:
                    # compute the bounding box for the part, then
                    # update our book-keeping variables
...
ap.add_argument("-r", "--reference", required=True, help="path to the reference MICR E-13B font")
...
# U = On-Us (delimit customer bank account number)
...
_, ref = cv2.threshold(ref, 0, 255, cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)

# find the contours in the MICR image (i.e., the outlines of the
...
# create a clone for the original image so we can draw on it
...
# extract the digits and symbols from the list of the contours, then
# initialize a dictionary to map the character name to the ROI
(refROIs, refLocs) = extract_digits_and_symbols(ref, refContours, minW=10, minH=20)
chars = {}
...
for (name, roi, loc) in zip(charNames, refROIs, refLocs):
     # draw a bounding box surrounding the character on the output image
     (xA, yA, xB, yB) = loc
...
# load the input image grab its dimension, and apply array slicing
...
# morphological operator to find dark regions against light
...
# the rest back into range [0, 255]
